"""
Sports Backtesting System - With API delay to avoid rate limiting
"""
...
class SportsBacktester:
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  Rate limited. Waiting {self.delay * (attempt + 1)}s...")
    ...
    def backtest_season(self, league_key, season):
        ...
        print(f"\n{'='*50}")
        print(f"📊 Testing {league_name} {season}")
        print(f"{'='*50}")
        ...
        for team_name in teams.keys():
            ...
            if team_id:
                ...
                team_count += 1
                if team_count % 5 == 0:
                    print(f"    Processed {team_count}/{len(teams)} teams...")
        
        print(f"  Found IDs for {team_count}/{len(teams)} teams")
    ...
    def run_all_seasons(self):
        ...
        # Only test one season at a time to avoid rate limits
        seasons = ["2024"]  # Start with just 2024
...
def main():
    print("=" * 60)
    print("🔶 SHADOW SPORTS BACKTESTER (WITH DELAYS)")
    print("   Testing 2024 season only to avoid rate limits")
    print("=" * 60)
    
    backtester = SportsBacktester()
    backtester.delay = 3  # 3 second delay between calls
