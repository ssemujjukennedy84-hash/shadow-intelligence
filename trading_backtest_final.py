"""
TRADING BACKTEST - FINAL FIXED
- Handles all confidence formats properly
"""
...
class TradingBacktester:
    ...
    def backtest_symbol(self, symbol, period="6mo"):
        ...
        for i in range(step, len(df) - self.forward_days, step):
            ...
            # FIXED: Handle all confidence formats
            confidence = result.get("confidence", 0)
            if isinstance(confidence, str):
                confidence = confidence.replace('%', '').strip()
                if confidence == "NO SIGNAL" or confidence == "" or confidence == "None":
                    continue
                try:
        ...
        if not trades:
            print(f"  No high-confidence trades (min {self.min_confidence}%)")
            return None
        ...
        print(f"  Avg Confidence: {sum(t['confidence'] for t in trades)/total:.0f}%")
    ...
    def run_all(self):
        ...
        print("=" * 60)
        print("🔶 TRADING BACKTEST - FINAL FIXED")
