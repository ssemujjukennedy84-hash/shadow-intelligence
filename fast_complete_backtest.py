"""
COMPLETE BACKTEST - FAST (No API calls)
Uses your offline data with weather/H2H from local cache
"""
...
def get_team_stats(team_name, matches):
    ...
    return {"name":team_name,"played":played,"wins":wins,"draws":draws,"losses":losses,
            "goals_for":gf,"goals_against":ga,"form":"".join(recent[-5:])}
...
for league_key in ["epl","laliga","bundesliga","seriea","ligue1"]:
    ...
    for idx, m in enumerate(matches):
        ...
        home_final={"name":home_name,"strength":home_scored.get("strength",5),
                   "energy":home_scored.get("energy",50),"morale":home_scored.get("morale",5)}
        away_final={"name":away_name,"strength":away_scored.get("strength",5),
                   "energy":away_scored.get("energy",50),"morale":away_scored.get("morale",5)}
    ...
    all_results.append({"league":league_name,"total":total,"correct":correct,"accuracy":acc})
...
for r in all_results:
    print(f"  {r['league']}: {r['correct']}/{r['total']} = {r['accuracy']:.1f}%")
print("="*60)
print(f"🏆 OVERALL: {total_correct}/{total_matches} = {total_correct/total_matches*100:.1f}%")
print("="*60)
